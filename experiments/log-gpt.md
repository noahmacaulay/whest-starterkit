...
## 2026-07-17T09:07:33Z - B46-gpt-20260717T085533Z: Shared-Haar signed-orbit blocks
- Hypothesis: one exact-Haar frame reused through 24 independent seeded Rademacher column-sign orbits preserves exact Haar marginals and unbiased radial-exact estimation while retaining useful cross-block coverage at roughly one QR rather than B43's 25 QR factorizations.
- Base champion: estimator.py @ 013df29 (B42-claude-lead-20260717T011419Z, source result ba03635); candidate_gpt.py checkpoint @ c81496b; claim commit 1381f00.
- Environment: whestbench=0.12.0rc3, flopscope=0.8.0rc5, uv.lock@9b677e2b91b0b4791c73c9449b6d5e5491093ddb.
- Evaluation: dataset=hf://aicrowd/arc-whestbench-public-2026@v1-phase1 (sha256=5b00938b6bd809fe80acef08772c5654edf467863225ca9e304b76c779ecf433), split=mini (100 MLPs), budget=272000000000, runner=subprocess. `uv run --frozen whest validate --estimator candidate_gpt.py` passed; both required `whest run` commands completed with exit code zero and their JSON parsed. Exact commands, hashes, raw reports, and paired calculations are in `experiments/results/gpt/B46-gpt-20260717T085533Z-013df29-summary.json`.
- Change: draw one seeded float32 Gaussian 256x256 matrix, obtain a sign-corrected exact-Haar frame with instrumented `fnp.linalg.qr`, reuse it as 25 blocks by applying 24 independent seeded Rademacher column diagonals, append 100 iid normalized directions, then use B42's 650-row float32 chunked forward with float64 accumulation and exact chi(256) mean-radius scaling.
- Result: candidate_score=5.345888128338e-07; champion_score=7.616174219616e-07; relative_change=-29.808747%; candidate final_layer_mse=5.016990630793e-06 vs champion=7.210819040324e-06 (-30.424122%); paired_mean_delta=-2.270286091278e-07; conservative paired_95pct_CI=[-4.476214655942e-07,-6.435752661455e-09] with tcrit=2.0; 55/100 improved and 45/100 regressed. Candidate mean FLOPs=2.7364338604e10 and effective compute=2.888252130066e10 vs champion 2.7349504000e10 and 2.874903520120e10. Worst adjusted-score regressions were +1.756319508482e-06 (`savannah-hayes`), +1.459126594531e-06 (`mary-nichols`), +6.828430959614e-07 (`kyle-braun`), +6.522597993643e-07 (`bobby-vasquez`), and +6.515330746077e-07 (`joseph-shaffer`). Both estimators have zero budget_exhausted, combined_budget_exhausted, time_exhausted, residual_wall_time_exhausted, error, and traceback flags.
- Verdict: PROMOTION-GATE PASS. The adjusted-score mean is negative and its conservative 95% CI is entirely below zero; the MSE-only paired CI is also entirely negative. Result persisted before promotion as required.
- Full/submission gate: NOT_RUN at result-persistence stage; promotion CAS must complete before any Full gate, reservation, or submission decision.
- New ideas queued: none.
